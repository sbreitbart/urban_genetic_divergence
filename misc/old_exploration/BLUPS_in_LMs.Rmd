...
tmp4 <- latex %>%
  dplyr::filter(!is.na(Latex_weight_mg))
predict(latex_fixed_LMM)
plot(tmp4$City_dist, predict(latex_fixed_LMM))
...
blup_pops_latex_fixed$estimate # these values are small- around 0.02- but they still seem biologically relevant.

# plot BLUPS vs. dist_city
plot(blup_pops_latex_fixed$City_dist, blup_pops_latex_fixed$estimate) # these are identity-linked, so no transformations needed. Suggests pretty small amount of variation among pops related to urbanization, which we see reflected in very small r-sq of city_dist.
...
plot(tmp$City_dist, predict(herb_early_binary_fixed_LMM))
```
...
# plot BLUPS vs. dist_city
plot(blup_pops_herb_early_bin$City_dist, blup_pops_herb_early_bin$estimate*1000) # range from -1 to 1, so must be logit-transformed. Transform w/inverse logit to get probabilities btwn 0-1

logit.x <- log(x/(1-x))
inv.logit.x <- exp(logit.x)/(1+exp(logit.x))

blup_pops_herb_early_bin$inv.logit <- exp(blup_pops_herb_early_bin$estimate*1000) / (1+exp(blup_pops_herb_early_bin$estimate*1000))

plot(blup_pops_herb_early_bin$City_dist, blup_pops_herb_early_bin$inv.logit) # these are between 0-1; specifically, 4.9x10^-9 to 5 x 10^-9. Super tiny. begs the question: biologically relevant?? or just negligible? (seems the latter)

# simpler linear model- convergence issue if I don't multiply response by large number. Estimates close to 0 otherwise
herb_early_binary_fixed_LM <- glmmTMB(estimate*1000 ~ 
                            City_dist,
                          data = blup_pops_herb_early_bin)
...
herb_late_binary_random_LM <- glmmTMB(estimate*1000 ~ 
                            City_dist,
...
tmp2 <- herbivory %>%
          dplyr::filter(Year != 2019 & !is.na(Herbivory_mean_late_binary))
plot(tmp2$City_dist, predict(herb_late_binary_fixed_LMM))

# inv.logit-transformed data
plot(tmp2$City_dist, predict(herb_late_binary_fixed_LMM))
tmp2$predict <- predict(herb_late_binary_fixed_LMM)
tmp2$inv.logit <- exp(tmp2$predict) / (1+exp(tmp2$predict))
plot(tmp2$City_dist, tmp2$inv.logit)
```
...
# plot BLUPS vs. dist_city
plot(blup_pops_herb_late_bin$City_dist, blup_pops_herb_late_bin$estimate*1000) # range from -1 to 1, so must be logit-transformed. Transform w/inverse logit to get probabilities btwn 0-1

logit.x <- log(x/(1-x))
inv.logit.x <- exp(logit.x)/(1+exp(logit.x))

blup_pops_herb_late_bin$inv.logit <- exp(blup_pops_herb_late_bin$estimate*1000) / (1+exp(blup_pops_herb_late_bin$estimate*1000))

plot(blup_pops_herb_late_bin$City_dist, blup_pops_herb_late_bin$inv.logit) # these are between 0-1; specifically, 4.9x10^-9 to 5 x 10^-9. Super tiny. begs the question: biologically relevant?? or just negligible? (seems the latter)


# simpler linear model- convergence issue
herb_late_binary_fixed_LM <- glmmTMB(estimate*1000 ~ 
                            City_dist,
                          data = blup_pops_herb_late_bin)
