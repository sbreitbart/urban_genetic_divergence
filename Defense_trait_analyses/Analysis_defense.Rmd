...
m1 <- lmer(LDMC ~ (1|Population/Family) + City_dist,
                             data = sla_ldmc %>%
                               dplyr::filter(LDMC < 1),
            REML = T) %T>%
  ranova()

m1.ranova <- as.data.frame(ranova(m1)) %>%
...
variances1 <- print(lme4::VarCorr(m1),
                    comp="Variance") %>%
  as.data.frame() %>%
...
full_join(m1.ranova, variances1) %>%
  as.data.frame() %>%
  dplyr::arrange(Group) %>%
  dplyr::select(c(1,8,9,7)) %>%
  dplyr::mutate_at(vars(c(2,4)), round, 3) %>%
  dplyr::rename(., p = 4) %>%
  dplyr::mutate(Variable = deparse(m1@call[["formula"]][[2]])) %>%
  dplyr::select(c(5, 1:4)) %>%
  flextable() %>%
  merge_at(j = 1) %>% 
  fix_border_issues() %>%
  save_as_html(path = here::here("./Defense_trait_analyses/Tables/Ranova/testingtoday.html"))
```
