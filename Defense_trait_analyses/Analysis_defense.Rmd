...
CreateRanovaOutput <- function(lmer_model){
...
tab1 <- full_join(m1.ranova, variances1) %>%
  ...
  dplyr::rename(., p = 4) %>%
  dplyr::mutate(Variable = deparse(lmer_model@call[["formula"]][[2]])) %>%
...
## Export: Gradient / City_dist
```{r}
# LDMC-------
ldmc_ranova_mod1 <- lmer(LDMC ~ (1|Population/Family) + City_dist,
                             data = sla_ldmc %>%
                               dplyr::filter(LDMC < 1),
                           REML = T)

ldmc_ranova1 <- CreateRanovaOutput(ldmc_ranova_mod1) %T>%
  flextable::save_as_html(.,
                        path = here::here("./Defense_trait_analyses/Tables/Ranova/ldmc.html"))


# SLA-----
sla_ranova_mod1 <- lmer(SLA ~ (1|Population/Family) + City_dist,
                        data = sla_ldmc,
                        REML = T)


sla_ranova1 <- CreateRanovaOutput(sla_ranova_mod1) %T>%
  flextable::save_as_html(.,
                        path = here::here("./Defense_trait_analyses/Tables/Ranova/sla.html"))


# Latex-----
latex_ranova_mod1 <- lmer(Latex_weight_mg ~ (1|Population/Family) + City_dist,
                        data = latex,
                        REML = T)

latex_ranova1 <- CreateRanovaOutput(latex_ranova_mod1) %T>%
  flextable::save_as_html(.,
...
## Export: Gradient / Urb_score
## Export: Urban Subtransects / City_dist
## Export: Urban Subtransects / Urb_score
