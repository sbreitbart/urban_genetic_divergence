# Load libraries & functions
```{r}
source("libraries.R")
source("functions.R")
```

# Import data
```{r}
herbivores_all <- read.csv(here::here("./Joined_annual_data/herbivores.csv")) %>%
  dplyr::select(., -1) %>%
  dplyr::mutate_at(vars(c("Population",
                          "Family",
                          "Replicate",
                          "Block",
                          "Transect_ID",
                          "Sample",
                          "Patch_ID",
                          "Urb_Rur",
                          "Year",
                          "Surveyor",
                          "Date")),
                   as.character) %>%
    dplyr::mutate_at(vars(c("Population",
                            "Family",
                            "Replicate",
                            "Block",
                            "Transect_ID",
                            "Sample",
                            "Patch_ID",
                            "Urb_Rur",
                            "Year",
                            "Surveyor",
                          "Date")),
                     as.factor)

str(herbivores_all)
```

# Create subset
```{r}
# filter out herbivores not surveyed in 2021
# Herbivores surveyed in 2020-2021: Monarchs, L. asclepiadis, L. clivicollis
herbivores <- herbivores_all %>%
  dplyr::select(c(1:7, 10, 13, 19, 22:Surveyor))%>%
  dplyr::mutate(Fam_uniq = as.factor(paste0(Population, "_", Family)))
```

# Diagnostics
## Monarchs
### Gradient
#### City_dist
```{r}
monarch_gr_city_m1 <- glmer(Monarch_Quantity_Observed ~
                                Block +
                                (1|Population/Family) +
                                Year +
                              #  Sample +
                                City_dist,
              data = herbivores,
              family = poisson

              ) # may be an outlier, but it's biologically relevant. And anova result doesn't change if it's included or excluded.

performance::check_model(monarch_gr_city_m1) # looks fine
car::Anova(monarch_gr_city_m1)


# # test for gxe
# gxe_test_mon1 <- glmer.nb(Monarch_Quantity_Observed ~
#                                 (1|Population/Family) +
#                                 Year +
#                                 Sample +
#                                 City_dist*Block,
#               data = herbivores
# 
#               )
# 
# test_gxe(gxe_test_mon1, monarch_gr_city_m1) # gxe mod best
# car::Anova(monarch_gr_city_m1, type = "III") # block sig here, not in gxe mod
# car::Anova(gxe_test_mon1, type = "III") # block*city NOT sig
```

#### Urb_score
```{r}
monarch_gr_usc_m1 <- glmer.nb(Monarch_Quantity_Observed ~
                                Block +
                                (1|Population/Family) +
                                Year +
                                Sample +
                                Urb_score,
              data = herbivores

              ) 

# performance::check_model(monarch_gr_usc_m1) # looks fine


# # test for gxe
# gxe_test_mon2 <- glmer.nb(Monarch_Quantity_Observed ~
#                                 (1|Population/Family) +
#                                 Year +
#                                 Sample +
#                                 Urb_score*Block,
#               data = herbivores
# 
#               )
# 
# test_gxe(gxe_test_mon2, monarch_gr_usc_m1) # gxe mod best
# car::Anova(monarch_gr_usc_m1, type = "III") # qual identical results
# car::Anova(gxe_test_mon2, type = "III") # block*urb_score NOT sig
```

### Urban Subtransects
#### City_dist
```{r}
# Full model
monarch_urbsubs_city_m1 <- glmer.nb(Monarch_Quantity_Observed ~
                                Block +
                                (1|Population/Family) +
                                Year +
                                Sample +
                                City_dist * Transect_ID,
              data = herbivores %>%
                dplyr::filter(Transect_ID != "Rural"),

              ) 

# performance::check_model(monarch_urbsubs_city_m1)


# MAIN EFFECTS MODEL
monarch_urbsubs_city_m2 <- glmer.nb(Monarch_Quantity_Observed ~
                                Block +
                                (1|Population/Family) +
                                Year +
                                Sample +
                                City_dist + Transect_ID,
              data = herbivores %>%
                dplyr::filter(Transect_ID != "Rural"),

              ) 

# performance::check_model(monarch_urbsubs_city_m2) # looks fine

# AIC(monarch_urbsubs_city_m1,
#     monarch_urbsubs_city_m2) # m2 better but <2 AIC apart


# # test for gxe
# gxe_test_mon3 <- glmer.nb(Monarch_Quantity_Observed ~
#                                 Block*City_dist +
#                                 (1|Population/Family) +
#                                 Year +
#                                 Sample +
#                                 City_dist * Transect_ID,
#               data = herbivores %>%
#                 dplyr::filter(Transect_ID != "Rural"),
# 
#               ) 
# 
# test_gxe(gxe_test_mon3, monarch_urbsubs_city_m1) # stat equiv
# car::Anova(monarch_urbsubs_city_m1, type = "III") # city_dist sig here, not in gxe mod
# car::Anova(gxe_test_mon3, type = "III") # block*city NOT sig
```

#### Urb_score
```{r}
# Full model
monarch_urbsubs_usc_m1 <- glmer.nb(Monarch_Quantity_Observed ~
                                Block +
                                (1|Population/Family) +
                                Year +
                                Sample +
                                Urb_score * Transect_ID,
              data = herbivores %>%
                dplyr::filter(Transect_ID != "Rural"),

              ) 

# performance::check_model(monarch_urbsubs_usc_m1)


# MAIN EFFECTS MODEL
monarch_urbsubs_usc_m2 <- glmer.nb(Monarch_Quantity_Observed ~
                                Block +
                                (1|Population/Family) +
                                Year +
                                Sample +
                                Urb_score + Transect_ID,
              data = herbivores %>%
                dplyr::filter(Transect_ID != "Rural"),

              ) 

# performance::check_model(monarch_urbsubs_usc_m2) # looks fine
# AIC(monarch_urbsubs_usc_m1,
#     monarch_urbsubs_usc_m2) # m2 better but <2 AIC apart


# # test for gxe
# gxe_test_mon4 <- glmer.nb(Monarch_Quantity_Observed ~
#                                 Block*Urb_score +
#                                 (1|Population/Family) +
#                                 Year +
#                                 Sample +
#                                 Urb_score * Transect_ID,
#               data = herbivores %>%
#                 dplyr::filter(Transect_ID != "Rural"),
# 
#               ) 
# 
# test_gxe(gxe_test_mon4, monarch_urbsubs_usc_m1) # stat equiv
# car::Anova(monarch_urbsubs_usc_m1, type = "III") 
# car::Anova(gxe_test_mon4, type = "III") # block*urb_score is sig (p = 0.04574)
# 
# 
# 
# # compare ranovas
# # GxE MOD-----
# #   test family
# #     large mod
# lgmod1 <- glmer.nb(Monarch_Quantity_Observed ~
#               #    Year +
#                   (1|Population) +
#                   (1|Population:Fam_uniq) +
#                   Urb_score*Block +
#                 Urb_score*Transect_ID,
#               data = herbivores %>%
#                 dplyr::filter(Year == "2021" &
#                                 Transect_ID != "Rural"),
#               control=glmerControl(optimizer="bobyqa",
#                              optCtrl=list(maxfun=2e5)),
#                             na.action = na.exclude
#               )
# 
# #     small mod 1
# smmod1 <- glmer.nb(Monarch_Quantity_Observed ~
#               #    Year +
#                   (1|Population) +
#                   Urb_score*Block +
#                 Urb_score*Transect_ID,
#                 data = herbivores %>%
#                 dplyr::filter(Year == "2021" &
#                                 Transect_ID != "Rural"),
#               control=glmerControl(optimizer="bobyqa",
#                              optCtrl=list(maxfun=2e5)),
#                             na.action = na.exclude
#               )
# 
# PBmodcomp(lgmod1, smmod1, seed = 45, nsim = 10) # p = 1
# 
# # test pop
# #     small mod 2
# smmod2 <- glmer.nb(Monarch_Quantity_Observed ~
#               #    Year +
#                   (1|Population:Fam_uniq) +
#                   Urb_score*Block +
#                 Urb_score*Transect_ID,
#                 data = herbivores %>%
#                 dplyr::filter(Year == "2021" &
#                                 Transect_ID != "Rural"),
#               control=glmerControl(optimizer="bobyqa",
#                              optCtrl=list(maxfun=2e5)),
#                             na.action = na.exclude
#               )
# 
# PBmodcomp(lgmod1, smmod2, seed = 45, nsim = 10) # p = 1
# 
# 
# # REG MOD-----
# #   test family
# #     large mod
# lgmod1.reg <- glmer.nb(Monarch_Quantity_Observed ~
#               #    Year +
#                   (1|Population) +
#                   (1|Population:Fam_uniq) +
#                   Urb_score*Transect_ID + Block,
#                 data = herbivores %>%
#                 dplyr::filter(Year == "2021" &
#                                 Transect_ID != "Rural"),
# 
#               control=glmerControl(optimizer="bobyqa",
#                              optCtrl=list(maxfun=2e5)),
#                             na.action = na.exclude
#               )
# 
# #     small mod 1
# smmod1.reg <- glmer.nb(Monarch_Quantity_Observed ~
#               #    Year +
#                   (1|Population) +
#                   Urb_score*Transect_ID + Block,
#                 data = herbivores %>%
#                 dplyr::filter(Year == "2021" &
#                                 Transect_ID != "Rural"),
# 
#               control=glmerControl(optimizer="bobyqa",
#                              optCtrl=list(maxfun=2e5)),
#                             na.action = na.exclude
#               )
# 
# PBmodcomp(lgmod1.reg, smmod1.reg, seed = 45, nsim = 10) # p = 1
# 
# # test pop
# #     small mod 2
# smmod2.reg <- glmer.nb(Monarch_Quantity_Observed ~
#              #     Year +
#                   (1|Population:Fam_uniq) +
#                   Urb_score*Transect_ID + Block,
#                 data = herbivores %>%
#                 dplyr::filter(Year == "2021" &
#                                 Transect_ID != "Rural"),
# 
#               control=glmerControl(optimizer="bobyqa",
#                              optCtrl=list(maxfun=2e5)),
#                             na.action = na.exclude
#              )
# 
# 
# PBmodcomp(lgmod1.reg, smmod2.reg, seed = 45, nsim = 10) # p = 1
# 
# # mod comparison-----
# # qualitatively identical so I'll keep using regular model.

```


## Liriomyza asclepiadis
### Gradient
#### City_dist
```{r}
asclepiadis_gr_city_m1 <- glmer.nb(Liriomyza_asclepiadis ~
                                Block +
                                (1|Population/Family) +
                                Year +
                                Sample +
                                City_dist,
              data = herbivores

              )

# performance::check_model(asclepiadis_gr_city_m1)


# # test for gxe
# gxe_test_ascl1 <- glmer.nb(Liriomyza_asclepiadis ~
#                                 (1|Population/Family) +
#                                 Year +
#                                 Sample +
#                                 City_dist*Block,
#               data = herbivores
# 
#               )
# 
# test_gxe(gxe_test_ascl1, asclepiadis_gr_city_m1) # gxe mod best
# car::Anova(asclepiadis_gr_city_m1, type = "III") # qual identical to gxe mod
# car::Anova(gxe_test_ascl1, type = "III") # block*city NOT sig
```

#### Urb_score
```{r}
asclepiadis_gr_usc_m1 <- glmer.nb(Liriomyza_asclepiadis ~
                                Block +
                                (1|Population/Family) +
                                Year +
                                Sample +
                                Urb_score,
              data = herbivores

              ) 

# performance::check_model(asclepiadis_gr_usc_m1)

# # test for gxe
# gxe_test_ascl2 <- glmer.nb(Liriomyza_asclepiadis ~
#                                 (1|Population/Family) +
#                                 Year +
#                                 Sample +
#                                 Urb_score*Block,
#               data = herbivores
# 
#               )
# 
# test_gxe(gxe_test_ascl2, asclepiadis_gr_usc_m1) # stat equiv
# car::Anova(asclepiadis_gr_usc_m1, type = "III") # qual identical to gxe mod
# car::Anova(gxe_test_ascl2, type = "III") # block*city NOT sig
```

### Urban Subtransects
#### City_dist
```{r}
# Full model
asclepiadis_urbsubs_city_m1 <- glmer.nb(Liriomyza_asclepiadis ~
                                Block +
                                (1|Population/Family) +
                                Year +
                                Sample +
                                City_dist * Transect_ID,
              data = herbivores %>%
                dplyr::filter(Transect_ID != "Rural"),

              ) 

# performance::check_model(asclepiadis_urbsubs_city_m1)


# MAIN EFFECTS MODEL
asclepiadis_urbsubs_city_m2 <- glmer.nb(Liriomyza_asclepiadis ~
                                Block +
                                (1|Population/Family) +
                                Year +
                                Sample +
                                City_dist + Transect_ID,
              data = herbivores %>%
                dplyr::filter(Transect_ID != "Rural"),

              ) 

# performance::check_model(asclepiadis_urbsubs_city_m2)
# AIC(asclepiadis_urbsubs_city_m1,
#     asclepiadis_urbsubs_city_m2) # m2 better but <2 AIC apart
#     
#     
# # test for gxe
# gxe_test_ascl3 <- glmer.nb(Liriomyza_asclepiadis ~
#                                 Block*City_dist +
#                                 (1|Population/Family) +
#                                 Year +
#                                 Sample +
#                                 City_dist * Transect_ID,
#               data = herbivores %>%
#                 dplyr::filter(Transect_ID != "Rural"),
# 
#               )
# 
# test_gxe(gxe_test_ascl3, asclepiadis_urbsubs_city_m1) # gxe mod best
# car::Anova(asclepiadis_urbsubs_city_m1, type = "III") # qual identical
# car::Anova(gxe_test_ascl3, type = "III") # block*city NOT sig
```

#### Urb_score
```{r}
# Full model
asclepiadis_urbsubs_usc_m1 <- glmer.nb(Liriomyza_asclepiadis ~
                                Block +
                                (1|Population/Family) +
                                Year +
                                Sample +
                                Urb_score * Transect_ID,
              data = herbivores %>%
                dplyr::filter(Transect_ID != "Rural"),

              )

# performance::check_model(asclepiadis_urbsubs_usc_m1)


# MAIN EFFECTS MODEL
asclepiadis_urbsubs_usc_m2 <- glmer.nb(Liriomyza_asclepiadis ~
                                Block +
                                (1|Population/Family) +
                                Year +
                                Sample +
                                Urb_score + Transect_ID,
              data = herbivores %>%
                dplyr::filter(Transect_ID != "Rural"),

              ) 

# performance::check_model(asclepiadis_urbsubs_usc_m2)
# AIC(asclepiadis_urbsubs_usc_m1,
#     asclepiadis_urbsubs_usc_m2) # m2 better but <2 AIC apart

    
# # test for gxe
# gxe_test_ascl4 <- glmer.nb(Liriomyza_asclepiadis ~
#                                 Block*Urb_score +
#                                 (1|Population/Family) +
#                                 Year +
#                                 Sample +
#                                 Urb_score * Transect_ID,
#               data = herbivores %>%
#                 dplyr::filter(Transect_ID != "Rural"),
# 
#               )
# 
# test_gxe(gxe_test_ascl4, asclepiadis_urbsubs_usc_m1) # gxe mod best
# car::Anova(asclepiadis_urbsubs_usc_m1, type = "III") # 
# car::Anova(gxe_test_ascl4, type = "III") # block*urbscore marg sig (p = 0.05128)


# # compare ranovas
# # GxE MOD-----
# #   test family
# #     large mod
# lgmod1 <- glmer.nb(Liriomyza_asclepiadis ~
#               #    Year +
#                   (1|Population) +
#                   (1|Population:Fam_uniq) +
#                   Urb_score*Block +
#                 Urb_score*Transect_ID,
#               data = herbivores %>%
#                 dplyr::filter(Year == "2021" &
#                                 Transect_ID != "Rural"),
#               control=glmerControl(optimizer="bobyqa",
#                              optCtrl=list(maxfun=2e5)),
#                             na.action = na.exclude
#               )
# 
# #     small mod 1
# smmod1 <- glmer.nb(Liriomyza_asclepiadis ~
#               #    Year +
#                   (1|Population) +
#                   Urb_score*Block +
#                 Urb_score*Transect_ID,
#                 data = herbivores %>%
#                 dplyr::filter(Year == "2021" &
#                                 Transect_ID != "Rural"),
#               control=glmerControl(optimizer="bobyqa",
#                              optCtrl=list(maxfun=2e5)),
#                             na.action = na.exclude
#               )
# 
# PBmodcomp(lgmod1, smmod1, seed = 45, nsim = 10) # p = 0.03392
# 
# # test pop
# #     small mod 2
# smmod2 <- glmer.nb(Liriomyza_asclepiadis ~
#               #    Year +
#                   (1|Population:Fam_uniq) +
#                   Urb_score*Block +
#                 Urb_score*Transect_ID,
#                 data = herbivores %>%
#                 dplyr::filter(Year == "2021" &
#                                 Transect_ID != "Rural"),
#               control=glmerControl(optimizer="bobyqa",
#                              optCtrl=list(maxfun=2e5)),
#                             na.action = na.exclude
#               )
# 
# PBmodcomp(lgmod1, smmod2, seed = 45, nsim = 10) # p = 0.7970
# 
# 
# # REG MOD-----
# #   test family
# #     large mod
# lgmod1.reg <- glmer.nb(Liriomyza_asclepiadis ~
#               #    Year +
#                   (1|Population) +
#                   (1|Population:Fam_uniq) +
#                   Urb_score*Transect_ID + Block,
#                 data = herbivores %>%
#                 dplyr::filter(Year == "2021" &
#                                 Transect_ID != "Rural"),
# 
#               control=glmerControl(optimizer="bobyqa",
#                              optCtrl=list(maxfun=2e5)),
#                             na.action = na.exclude
#               )
# 
# #     small mod 1
# smmod1.reg <- glmer.nb(Liriomyza_asclepiadis ~
#               #    Year +
#                   (1|Population) +
#                   Urb_score*Transect_ID + Block,
#                 data = herbivores %>%
#                 dplyr::filter(Year == "2021" &
#                                 Transect_ID != "Rural"),
# 
#               control=glmerControl(optimizer="bobyqa",
#                              optCtrl=list(maxfun=2e5)),
#                             na.action = na.exclude
#               )
# 
# PBmodcomp(lgmod1.reg, smmod1.reg, seed = 45, nsim = 10) # p = 0.038
# 
# # test pop
# #     small mod 2
# smmod2.reg <- glmer.nb(Liriomyza_asclepiadis ~
#              #     Year +
#                   (1|Population:Fam_uniq) +
#                   Urb_score*Transect_ID + Block,
#                 data = herbivores %>%
#                 dplyr::filter(Year == "2021" &
#                                 Transect_ID != "Rural"),
# 
#               control=glmerControl(optimizer="bobyqa",
#                              optCtrl=list(maxfun=2e5)),
#                             na.action = na.exclude
#              )
# 
# 
# PBmodcomp(lgmod1.reg, smmod2.reg, seed = 45, nsim = 10) # p = 0.8774
# 
# # mod comparison-----
# # qualitatively identical so I'll keep using regular model.
```


## Labidomera clivicollis
### Gradient
#### City_dist
```{r}
clivicollis_gr_city_m1 <- glmer.nb(Labidomera_clivicollis ~
                                Block +
                                (1|Population/Family) +
                                Year +
                                Sample +
                                City_dist,
              data = herbivores %>%
                dplyr::filter(Labidomera_clivicollis < 9),

              ) # remove outliers to maintain homogeneity of variance

# performance::check_model(clivicollis_gr_city_m1)


# # test for gxe
# gxe_test_cliv1 <- glmer.nb(Labidomera_clivicollis ~
#                                 (1|Population/Family) +
#                                 Year +
#                                 Sample +
#                                 City_dist*Block,
#               data = herbivores %>%
#                 dplyr::filter(Labidomera_clivicollis < 9),
# 
#               )
# 
# test_gxe(gxe_test_cliv1, clivicollis_gr_city_m1) # stat equiv
# car::Anova(clivicollis_gr_city_m1, type = "III") # otherwise, qual identical to gxe mod
# car::Anova(gxe_test_cliv1, type = "III") # block*city marg sig (p = 0.0998624)


# # compare ranovas
# # GxE MOD-----
# #   test family
# #     large mod
# lgmod1 <- glmer.nb(Labidomera_clivicollis ~
#               #    Year +
#                   (1|Population) +
#                   (1|Population:Fam_uniq) +
#                   City_dist*Block,
#                data = herbivores %>%
#                 dplyr::filter(Labidomera_clivicollis < 9 &
#                                 Year == "2021" &
#                                 is.na(Labidomera_clivicollis) == FALSE),
#              nAGQ = 0, # this mod can converge w/o this, but then it wouldn't be comparable to the regular model below
#               na.action = na.exclude
#               )
# 
# #     small mod 1
# smmod1 <- glmer.nb(Labidomera_clivicollis ~
#               #    Year +
#                   (1|Population) +
#                   City_dist*Block,
#                 data = herbivores %>%
#                 dplyr::filter(Labidomera_clivicollis < 9 &
#                                 Year == "2021"&
#                                 is.na(Labidomera_clivicollis) == FALSE),
#               nAGQ = 0, 
#                             na.action = na.exclude
#               )
# 
# PBmodcomp(lgmod1, smmod1, seed = 45, nsim = 10) # p = 1
# 
# # test pop
# #     small mod 2
# smmod2 <- glmer.nb(Labidomera_clivicollis ~
#               #    Year +
#                   (1|Population:Fam_uniq) +
#                   City_dist*Block,
#                 data = herbivores %>%
#                 dplyr::filter(Labidomera_clivicollis < 9 &
#                                 Year == "2021"&
#                                 is.na(Labidomera_clivicollis) == FALSE),
#              nAGQ = 0,
#                             na.action = na.exclude
#               )
# 
# PBmodcomp(lgmod1, smmod2, seed = 45, nsim = 10) # p = 0.6701. I had to change nAGQ to 0 for these models, or else it wouldn't work
# 
# 
# # REG MOD-----
# #   test family
# #     large mod
# lgmod1.reg <- glmer.nb(Labidomera_clivicollis ~
#               #    Year +
#                   (1|Population) +
#                   (1|Population:Fam_uniq) +
#                 City_dist +
#                 Block,
#                 data = herbivores %>%
#                 dplyr::filter(Labidomera_clivicollis < 9 &
#                                 Year == "2021"&
#                                 is.na(Labidomera_clivicollis) == FALSE),
#                             na.action = na.exclude,
#               nAGQ = 0 # doesn't converge w/o this
#               )
# 
# #     small mod 1
# smmod1.reg <- glmer.nb(Labidomera_clivicollis ~
#               #    Year +
#                   (1|Population) +
#                 City_dist + 
#                 Block,
#                 data = herbivores %>%
#                 dplyr::filter(Labidomera_clivicollis < 9 &
#                                 Year == "2021"&
#                                 is.na(Labidomera_clivicollis) == FALSE),
#                             nAGQ = 0, # doesn't converge w/o this
#                             na.action = na.exclude
#               )
# 
# PBmodcomp(lgmod1.reg, smmod1.reg, seed = 45, nsim = 10) # p = 1
# 
# # test pop
# #     small mod 2
# smmod2.reg <- glmer.nb(Labidomera_clivicollis ~
#              #     Year +
#                   (1|Population:Fam_uniq) +
#                    + Block,
#                 data = herbivores %>%
#                 dplyr::filter(Labidomera_clivicollis < 9 &
#                                 Year == "2021" &
#                                 is.na(Labidomera_clivicollis) == FALSE),
#               nAGQ = 0, 
#                             na.action = na.exclude
#              )
# 
# 
# PBmodcomp(lgmod1.reg, smmod2.reg, seed = 45, nsim = 10) # p = 0.9843
# 
# # mod comparison-----
# # qualitatively identical so I'll keep using regular model.
```

#### Urb_score
```{r}
clivicollis_gr_usc_m1 <- glmer.nb(Labidomera_clivicollis ~
                                Block +
                                (1|Population/Family) +
                                Year +
                                Sample +
                                Urb_score,
              data = herbivores %>%
                dplyr::filter(Labidomera_clivicollis < 9),

              ) # remove outliers to maintain homogeneity of variance

# performance::check_model(clivicollis_gr_usc_m1)


# # test for gxe
# gxe_test_cliv2 <- glmer.nb(Labidomera_clivicollis ~
#                                 (1|Population/Family) +
#                                 Year +
#                                 Sample +
#                                 Urb_score*Block,
#               data = herbivores %>%
#                 dplyr::filter(Labidomera_clivicollis < 9),
# 
#               )
# 
# test_gxe(gxe_test_cliv2, clivicollis_gr_city_m1) # stat equiv
# car::Anova(clivicollis_gr_city_m1, type = "III") # qual identical
# car::Anova(gxe_test_cliv2, type = "III") 
```

### Urban Subtransects
#### City_dist
```{r}
# Full model
clivicollis_urbsubs_city_m1 <- glmer.nb(Labidomera_clivicollis ~
                                Block +
                                (1|Population/Family) +
                                Year +
                                Sample +
                                City_dist * Transect_ID,
              data = herbivores %>%
                dplyr::filter(Transect_ID != "Rural" &
                                Labidomera_clivicollis < 20), # remove outlier

              )

# performance::check_model(clivicollis_urbsubs_city_m1) 



# MAIN EFFECTS MODEL
clivicollis_urbsubs_city_m2 <- glmer.nb(Labidomera_clivicollis ~
                                Block +
                                (1|Population/Family) +
                                Year +
                                Sample +
                                City_dist + Transect_ID,
              data = herbivores %>%
                dplyr::filter(Transect_ID != "Rural" & Labidomera_clivicollis < 20), # remove outlier

              )

# performance::check_model(clivicollis_urbsubs_city_m2) 
# AIC(clivicollis_urbsubs_city_m1,
#     clivicollis_urbsubs_city_m2) # m1 better but <2 AIC apart



# # test for gxe
# gxe_test_cliv3 <- glmer.nb(Labidomera_clivicollis ~
#                                 Block*City_dist +
#                                 (1|Population/Family) +
#                                 Year +
#                                 Sample +
#                                 City_dist * Transect_ID,
#               data = herbivores %>%
#                 dplyr::filter(Transect_ID != "Rural" &
#                                 Labidomera_clivicollis < 20), # remove outlier
# 
#               )
# 
# test_gxe(gxe_test_cliv3, clivicollis_urbsubs_city_m1) # stat equiv
# car::Anova(clivicollis_urbsubs_city_m1, type = "III") # city_dist sig in reg model, otherwise qual identical
# car::Anova(gxe_test_cliv3, type = "III")
```

#### Urb_score
```{r}
# Full model
clivicollis_urbsubs_usc_m1 <- glmer.nb(Labidomera_clivicollis ~
                                Block +
                                (1|Population/Family) +
                                Year +
                                Sample +
                                Urb_score * Transect_ID,
              data = herbivores %>%
                dplyr::filter(Transect_ID != "Rural" &
                                Labidomera_clivicollis < 20), # remove outlier

              )

# performance::check_model(clivicollis_urbsubs_usc_m1) 



# MAIN EFFECTS MODEL
clivicollis_urbsubs_usc_m2 <- glmer.nb(Labidomera_clivicollis ~
                                Block +
                                (1|Population/Family) +
                                Year +
                                Sample +
                                Urb_score + Transect_ID,
              data = herbivores %>%
                dplyr::filter(Transect_ID != "Rural" & Labidomera_clivicollis < 20), # remove outlier

              )

# performance::check_model(clivicollis_urbsubs_usc_m2) 
# AIC(clivicollis_urbsubs_usc_m1,
#     clivicollis_urbsubs_usc_m2) # m2 better but <2 AIC apart



# # test for gxe
# gxe_test_cliv4 <- glmer.nb(Labidomera_clivicollis ~
#                                 Block*Urb_score +
#                                 (1|Population/Family) +
#                                 Year +
#                                 Sample +
#                                 Urb_score * Transect_ID,
#               data = herbivores %>%
#                 dplyr::filter(Transect_ID != "Rural" &
#                                 Labidomera_clivicollis < 20), # remove outlier
# 
#               )
# 
# test_gxe(gxe_test_cliv4, clivicollis_urbsubs_usc_m1) # gxe mod best
# car::Anova(clivicollis_urbsubs_usc_m1, type = "III") # qual identical
# car::Anova(gxe_test_cliv4, type = "III")
```

# Models for use in analysis
## Monarchs
```{r}
monarch_mods <- list(

## City_dist / gradient
monarch_gr_city_m1 ,

## Urbanization score / gradient
monarch_gr_usc_m1 ,

## City_dist / urban subtransects
monarch_urbsubs_city_m1, # Qualitatively identical model (<2 AIC away)
monarch_urbsubs_city_m2, # Best model

## Urbanization score  / urban subtransects
monarch_urbsubs_usc_m1, # Qualitatively identical model (<2 AIC away)
monarch_urbsubs_usc_m2 # Best model

)

names(monarch_mods) <- c("City_gr",
                     "Usc_gr",
                     "City_urbsubs_alt",
                     "City_urbsubs_best",
                     "Usc_urbsubs_alt",
                     "Usc_urbsubs_best")
```

## L. asclepiadis
```{r}
asclepiadis_mods <- list(

## City_dist / gradient
asclepiadis_gr_city_m1 ,

## Urbanization score / gradient
asclepiadis_gr_usc_m1 ,

## City_dist / urban subtransects
asclepiadis_urbsubs_city_m1, # Qualitatively identical model (<2 AIC away)
asclepiadis_urbsubs_city_m2, # Best model

## Urbanization score  / urban subtransects
asclepiadis_urbsubs_usc_m1, # Qualitatively identical model (<2 AIC away)
asclepiadis_urbsubs_usc_m2 # Best model

)

names(asclepiadis_mods) <- c("City_gr",
                     "Usc_gr",
                     "City_urbsubs_alt",
                     "City_urbsubs_best",
                     "Usc_urbsubs_alt",
                     "Usc_urbsubs_best")
```

## L. clivicollis
```{r}
clivicollis_mods <- list(

## City_dist / gradient
clivicollis_gr_city_m1 ,

## Urbanization score / gradient
clivicollis_gr_usc_m1 ,

## City_dist / urban subtransects
clivicollis_urbsubs_city_m1, # Best model
clivicollis_urbsubs_city_m2, # Qualitatively identical model (<2 AIC away)

## Urbanization score  / urban subtransects
clivicollis_urbsubs_usc_m1, # Qualitatively identical model (<2 AIC away)
clivicollis_urbsubs_usc_m2 # Best model

)

names(clivicollis_mods) <- c("City_gr",
                     "Usc_gr",
                     "City_urbsubs_best",
                     "City_urbsubs_alt",
                     "Usc_urbsubs_alt",
                     "Usc_urbsubs_best")
```