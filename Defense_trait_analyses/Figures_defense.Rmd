...
herbivory$Herbivory_mean_early_recode_cuberoot
...
ggplot(weevil %>%
         dplyr::filter(Transect_ID != "Rural" &
                         Scar_length_cm_cuberoot != 0),
       aes(x = Transect_ID,
           y = Scar_length_cm_cuberoot)) + 
  geom_boxplot(
       size = 0.5) + 
  geom_point(position = "jitter") + 
  theme_pubclean()
...
#### Herbivory: Before flowering
```{r}
# PERCENT CHANGE, rural to urban terminus:
Calc_percent_change(herb_e_gradient_01_pred) # 19.7%
```
...
```{r}
# PERCENT CHANGE, suburban to urban terminus:
Calc_percent_change_urbsubs(herb_e_urbsubs_01_pred) # 26.9%
```

#### Herbivory: After flowering
```{r}
Calc_percent_change_urbsubs_intxn(herb_l_urbsubs_01_pred)
# "Percent change from suburban to urban terminus along         corridor subtransect: 20.7%"
# "Percent change from suburban to urban terminus along         non-corridor subtransect: 34.2%"
```
