...
clivicollis_ranova.pop <- PBmodcomp(full, reduced,
          nsim = 100,
          cl = makeCluster(7))
tidy(clivicollis_ranova.pop) #  p ~ 0.
...
clivicollis_ranova.fam <- PBmodcomp(full, reduced,
          nsim = 100,
          cl = makeCluster(6))
tidy(clivicollis_ranova.fam) # Fam: p ~ 0.
...
full <- glmer(Labidomera_clivicollis ~
              ...
              family = nbinom2,
             na.action = na.exclude # THIS LINE IS IMPORTANT
  ) # typically doesn't converge/work well if response is transformed, so will keep it untransformed. Also didn't converge with year AND block included
...
tidy(clivicollis_ranova.pop) # Pop: p ~ 0.002
