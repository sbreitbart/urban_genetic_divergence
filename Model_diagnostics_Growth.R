...
# MAIN EFFECTS MODEL
# have to remove block for it to converge
...
sla_gr_city_m1 <- glmmTMB(log(SLA+1) ~
                            Block +
                            (1|Population/Family) +
                            City_dist,
                        data = sla_ldmc,
...
# test for gxe intxns
# gxe_test_sla <- glmmTMB(log(SLA+1) ~
#                              City_dist*Block +
#                              (1|Population/Family),
#                          data = sla_ldmc ,
#                         REML = F)
...
# test for gxe intxns
# gxe_test_sla2 <- glmmTMB(log(SLA +1) ~
#                              Urb_score*Block +
#                              (1|Population/Family),
#                          data = sla_ldmc ,
#                         REML = F)
...
# MAIN EFFECTS
sla_urbsubs_city_m2 <- glmmTMB(log(SLA+1) ~
                                 Block +
                                 (1|Population/Family) +
                                 City_dist + Transect_ID,
                        data = sla_ldmc %>%
...
# test for gxe intxns
# gxe_test_sla3 <- glmmTMB(log(SLA+1) ~
#                              City_dist*Block +
#                              (1|Population/Family) +
#                         City_dist*Transect_ID,
#                         sla_ldmc %>%
...
# MAIN EFFECTS
sla_urbsubs_usc_m2 <- glmmTMB(log(SLA+1) ~
                                 Block +
                                 (1|Population/Family) +
                                 Urb_score + Transect_ID,
                        data = sla_ldmc %>%
...
# test for gxe intxns
# gxe_test_sla4 <- glmmTMB(log(SLA+1) ~
#                              Urb_score*Block +
#                              (1|Population/Family) +
#                         Urb_score*Transect_ID,
#                         sla_ldmc %>%
...
# # GxE MOD
# ranova(lmer(rel_growth_rate^(1/3) ~
#                              Urb_score*Block +
#                         #     Year +
#                              (1|Population/Family) +
#                              Urb_score*Transect_ID,
...
#                          REML = F))
# # p(Population) = 0.9111; p(Family) = 1
# 
# # REG MOD
# ranova(lmer(rel_growth_rate^(1/3) ~
#                                  Block +
...
#                           dplyr::filter(Transect_ID != "Rural" & Year == "2021"),
#                          REML = F))
# # p(Population) = 0.9819; p(Family) = 1
# #
# # p-values for both mods qualitatively identical, so I'll keep using regular model. Results identical when year is added.
...
#                 nAGQ = 0)
# 
# PBmodcomp(lgmod1, smmod2, seed = 45, nsim = 10) # p = 0.8846
# 
# 
# # REG MOD-----
# #   test family
# #     large mod
# lgmod1.reg <- glmer(Ramets_early ~
#               #    Year +
#                   (1|Population) +
#                   (1|Population:Fam_uniq) +
#                   City_dist + Block,
...
#                 nAGQ = 0)
# 
# PBmodcomp(lgmod1.reg, smmod2.reg, seed = 45, nsim = 10) # p = 0.9344
# # mod comparison-----
# # qualitatively identical so I'll keep using regular model. Results identical when year is added.
...
#                             optCtrl=list(maxfun=2e5))) # won't converge w/o this
# 
# 
# #     small mod 1
# smmod1 <- glmer(Dead ~
...
#                             optCtrl=list(maxfun=2e5))) # won't converge w/o this
# 
# PBmodcomp(lgmod1, smmod1, seed = 45, nsim = 10) # p = 0.03827
# 
# # test pop
# #     small mod 2
# smmod2 <- glmer(Dead ~
...
# 
# PBmodcomp(lgmod1, smmod2, seed = 45, nsim = 10) # p = 0.9999
# 
# 
# # REG MOD-----
# #   test family
# #     large mod
# lgmod1.reg <- glmer(Dead ~
#                #   Year +
#                   (1|Population) +
#                   (1|Population:Fam_uniq) +
#                   Urb_score + Block+
...
#                             optCtrl=list(maxfun=2e5)))
# 
# PBmodcomp(lgmod1.reg, smmod1.reg, seed = 45, nsim = 10) # p = 0.04645
# 
# # test pop
# #     small mod 2
# smmod2.reg <- glmer(Dead ~
#               #    Year +
#                   (1|Population:Fam_uniq) +
#                   Urb_score + Block+
#                             Urb_score*Transect_ID,
