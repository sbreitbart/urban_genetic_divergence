...
```{r}
# LDMC-------
# even after I remove populations with only 1 family, it's still singular
# But when I remove FAMILY, it isn't singular! Makes sense b/c
# there's no variance among families as per the summary
...
summary(ldmc_ranova_mod1) # this shows variance among families and populations nearly 0; on the order of 1x10^-6 or plain 0.
summary(sla_ldmc$LDMC)
ranova(ldmc_ranova_mod1)
# shows very little variance among fams and pops too w/glmmtmb
summary(glmmTMB(LDMC ~
                  (1|Population/Family) +
                  Block,
                data = sla_ldmc %>%
                  dplyr::filter(LDMC < 0.7 &
...
# find fams w/1 replicate
ldmc_sum <- sla_ldmc %>%
  dplyr::filter(is.na(LDMC) ==F) %>%
  dplyr::group_by(Population, Family) %>%
  dplyr::summarise(reps = n()) %T>%
  view()
ldmc_sum2 <- sla_sum %>%
...
# SLA-----
# this is even singular untransformed and w/only SLA ~ (1|Pop)
# but it's NOT singular w/glmmTMB!
...
summary(sla_ldmc)
plot(sla_ldmc$SLA)
summary(sla_ranova_mod1) # this shows variance among families and populations is 0
...
## Put all tables into one document
```{r}
