...
sla_sum <- sla_ldmc %>%
  ...
  dplyr::summarise(reps = unique(Replicate))
...
```{r}
files <- list.files(path = "./Reproductive_trait_analyses/Figures/ANOVA/Best_models", full.names = T)
...
grid(images[[1]])
lapply(images[-1], function(x) {grid.newpage(); grid.raster(x)} ) -> bquiet
dev.off()
