...
tidy(ramets_e_ranova.pop) # Pop: p ~ 0.11
...
# merge Pop and Fam ranovas
## test city_dist
ramets_e_ranova1 <- ramets_e_ranova.pop %>%
  tidy() %>%
  ...
  as.data.frame() %>%
    ...
    dplyr::mutate(p = replace(p,
                              p < 0.001,
                              "<0.001")) %>%
    dplyr::mutate(Variable = noquote("Ramets, before flowering: (Distance)")) %>%
...
tidy(surv_ranova.fam) # Family: p ~ 0.
...
surv_mean <- survival_seasons %>%
  as.data.frame %>%
  dplyr::summarize(Mean = mean(seasons ,
                               na.rm = T)) %>%
  as.numeric()
...
qg_survival <- calc_quantgenvars(6.1473e-07,
                   1.3817e-05,
                   resid_var ,
                   surv_mean,
                   "Seasons survived")
