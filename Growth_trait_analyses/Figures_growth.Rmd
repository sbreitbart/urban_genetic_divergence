...
```{r}
# city_dist
mortality_urbsubs_01_pred <- ggeffects::ggpredict(surv_urbsubs_city_m2_seasons,
                                   terms = c("City_dist", "Transect_ID"),
                                   type = "fe") %>%
  as.data.frame()

# urb_score
mortality_urbsubs_02_pred <- ggeffects::ggpredict(surv_urbsubs_usc_m2_seasons,
                                   terms = c("Urb_score", "Transect_ID"),
                                   type = "fe")
...
dplyr::select(p))),
size = 4.5) +
  ggpp::geom_text_npc(aes(npcx = 1, npcy = 0.79,
                    label = paste0(" U x S: p = ", tidy_anova(rgr_mods_best_u[[2]]) %>%
