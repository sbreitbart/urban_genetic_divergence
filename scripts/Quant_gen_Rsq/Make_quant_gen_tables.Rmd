...
flsucc_mean <- reproduc %>%
  dplyr::summarize(Mean = mean(as.numeric(Flowered) + 0.00000000001 ,
                               na.rm = T)) %>%
  as.numeric() # can't calculate this because it's 0s x 1s... will be 0, so adding a tiny number to increase it above 0 for an estimate
