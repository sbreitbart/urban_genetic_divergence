...
manually extracted, then organized into csvs for import into R and manipulation: 
...
# Calculate heritability, Qst, and CVg (Quantitative genetic parameters)
...
flsucc_mean <- reproduc %>%
  dplyr::summarize(Mean = mean(as.numeric(Flowered) ,
                               na.rm = T)) %>%
  as.numeric() 
...
poll_mean <- flowering %>%
  dplyr::summarize(Mean = mean(mean_poll,
                               na.rm = T)) %>%
  as.numeric()

qg_poll <- calc_quantgenvars(2.1425e-01   ,
                   1.0387e-05  ,
                   3.2508e-01,
                   poll_mean,
                   "Pollinaria removed")
...
first_pods_mean <- flowering %>%
  dplyr::summarize(Mean = mean(Julian_first_follicle -170,
                               na.rm = T)) %>%
  as.numeric()
...
qg_monarch <- calc_quantgenvars(0.052469,
                                0.074502,
                                resid_var,
                                monarch_mean,
...
qg_lasclep <- calc_quantgenvars(0.25752  ,
            0.12421  ,
            resid_var,
            lascelp_mean,
...
qg_clivicoll <- calc_quantgenvars(0.50549 ,
            0.48033 ,
            resid_var,
            clivicoll_mean,
...
qg_ldmc <- calc_quantgenvars( 3.7986e-06,
                   8.1643e-07,
                   5.8917e-02,
                   ldmc_mean,
                   "LDMC")
...
qg_sla <- calc_quantgenvars(8.5713e-06,
                  2.4052e-02,
                    3.4704e-01,
                   sla_mean,
                   "SLA")
...
qg_heights_e <- calc_quantgenvars(0.215134,
                                  0.039398 ,
                                  0.861319 ,
                   heights_e_mean,
...
rgr_mean <- heights %>%
  dplyr::summarize(Mean = mean(rel_growth_rate*100, # multiplying by 100 because that's how I transformed it in model
                               na.rm = T)) %>%
  as.numeric()
 
qg_rgr <- calc_quantgenvars(0.00057931 ,
                            0.26399752 ,
                            5.54025753, 
                   rgr_mean,
...
qg_survival <- calc_quantgenvars(0.86406 ,
                  0.40162 ,
                   resid_var ,
                   surv_mean,
                   "Mortality")
...
qg_1yr_poll <- calc_quantgenvars(0.160682  ,
                   0.030858    ,
                   0.261111,
                   poll_mean,
                   "Pollinaria removed")
...
qg_1yr_monarch <- calc_quantgenvars(0.24206  ,
                                0.11298 ,
                                resid_var,
                                monarch_mean,
                                "D. plexippus abundance")
...
qg_1yr_lasclep <- calc_quantgenvars(0.19164   ,
            0.16830   ,
            resid_var,
            lascelp_mean,
            "L. asclepiadis abundance")
...
qg_1yr_clivicoll <- calc_quantgenvars(1.1656e-04,
            8.7063e-05,
            resid_var,
            clivicoll_mean,
            "L. clivicollis abundance")
...
qg_1yr_heights_e <- calc_quantgenvars(2.2422e-01 ,
                                  7.5154e-05  ,
                                  1.1883e+00  ,
                   heights_e_mean,
                   "Height before flowering")
...
qg_1yr_rgr <- calc_quantgenvars(0.62258438,
                            0.00072807,
                            6.03911527,
                   rgr_mean,
                   "Relative growth rate")
...
summarise(qgen_df,
          ...
          range_cvg = range(cvg),
          se_h2 = std.error(H2),
          se_qst = std.error(qst),
          se_cvg = std.error(cvg)) 


```


#### Take p-values from ranova (without urb) and append to qgen table
```{r}
...
# round p-value columns to 3 digits if not <0.001
qgen_list1$Family_p <- ifelse(qgen_list1$Family_p == "<0.001",
                        qgen_list1$Family_p,
                        round(as.numeric(qgen_list1$Family_p), 3))

qgen_list1$Population_p <- ifelse(qgen_list1$Population_p == "<0.001",
                        qgen_list1$Population_p,
                        round(as.numeric(qgen_list1$Population_p), 3))

qgen_list1 %>% 
  flextable() %>%
  hline(border = NULL, part = "body") %>%
  flextable::bold(i = ~ Family_p <= 0.05, j = 4) %>%
  flextable::bold(i = ~ Population_p <= 0.05, j = 6) %>%
  flextable::compose(part = "header", i = 1, j = 2,
         value = as_paragraph("H",
                              as_sup("2"))) %>%
  ...
  autofit() %>%
    padding(padding = 3) %>%
  save_as_docx(path = here::here("./Figures_Tables/Quant_gen/Quant_gen_1yrmods_withRanova.docx"))
```
