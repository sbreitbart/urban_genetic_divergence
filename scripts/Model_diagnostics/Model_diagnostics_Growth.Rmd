...
# Import data
```{r}
sla_ldmc <- read.csv(here::here("./data/CommonGardenExperiment_2020Data/clean_data/2020_sla_ldmc_clean.csv")) %>%
  dplyr::select(., -c(1:2)) %>%
...
heights <- read.csv(here::here("./data/Joined_annual_data/heights.csv"), na.strings=c("NO PLANT", "none"), blank.lines.skip=TRUE, header=TRUE, sep=",") %>%
  dplyr::select(., -1) %>%
  ...
  dplyr::mutate_at(c("Total_Height_early", "Total_Height_late", "rel_growth_rate"), as.numeric) %>%
  dplyr::mutate(Fam_uniq = paste0(Population, "_", Family))
...
#### City_dist
```{r}
ldmc_urbsubs_city_m1 <- glmmTMB(sqrt(LDMC) ~
                        #      Block +
                             (1|Population/Family) +
...
# MAIN EFFECTS MODEL
# have to remove block to converge
ldmc_urbsubs_city_m2 <- glmmTMB(LDMC^(1/2) ~
                           #   Block +
                             (1|Population/Family) +
                             City_dist + Transect_ID,
                        data = sla_ldmc %>%
...
# test for gxe intxns
# gxe_test_ldmc3 <- glmmTMB(sqrt(LDMC) ~
#                              City_dist*Block +
#                              (1|Population/Family) +
#                         City_dist*Transect_ID,
#                         sla_ldmc %>%
...
# test for gxe intxns
# gxe_test_height_e <- glmmTMB(Total_Height_early^(1/3) ~
#                                  City_dist*Block +
#                                  Year +
#                                  (1|Population/Family)
#                                  ,
...
#                         REML = F)
# 
# test_gxe(height_e_gr_city_m1, gxe_test_height_e) # reg mod best
```
...
#                         REML = F)
# 
# test_gxe(height_e_gr_usc_m1, gxe_test_height_e2) # reg mod best
```
...
#                           REML = F)
# 
# test_gxe(height_e_urbsubs_usc_m1, gxe_test_height_e4) # gxe mod best
# car::Anova(gxe_test_height_e4, type = "III") # Urb_score:Block sig at p = 0.0225
...
# # GxE MOD
# ranova(lmer((Total_Height_early^(1/3)) ~
#                              Urb_score*Block +
#                              (1|Population/Family) +
#                              Urb_score*Transect_ID,
#                          data = heights %>%
...
#                          REML = F))
# # p(Population) = 1; p(Family) = 0.3625
# 
# # REG MOD
# ranova(lmer((Total_Height_early^(1/3)) ~
#                                  Block +
...
#                           dplyr::filter(Transect_ID != "Rural" & Year == "2021"),
#                          REML = F))
# # p(Population) = 1; p(Family) = 0.296
# #
# # p-values for both mods qualitatively identical, so I'll keep using regular model. 
...
# doesn't converge unless I increase magnitude of RGR
rgr_gr_city_m1 <- glmmTMB((rel_growth_rate^(1/3))*100 ~ 
                                 Block +
                                 Year +
                                 (1|Population/Family) +
                                 City_dist,
...
# test for gxe intxns
# gxe_test_rgr <- glmmTMB((rel_growth_rate^(1/3))*100 ~
#                              City_dist*Block +
#                              (1|Population/Family),
#                          data = heights ,
#                         REML = F)
...
#### Urb_score
```{r}
rgr_gr_usc_m1 <- glmmTMB((rel_growth_rate^(1/3))*100 ~ 
                                 Block +
                                 Year +
                                 (1|Population/Family) +
                                 Urb_score,
...
# test for gxe intxns
# gxe_test_rgr2 <- glmmTMB((rel_growth_rate^(1/3))*100 ~
#                              Urb_score*Block +
#                              (1|Population/Family),
#                          data = heights ,
#                         REML = F)
...
#### City_dist
```{r}
rgr_urbsubs_city_m1 <- glmmTMB((rel_growth_rate^(1/3))*100 ~ 
                                 Block +
                                 Year +
                                 (1|Population/Family) +
                                 City_dist * Transect_ID,
...
# MAIN EFFECTS MODEL
rgr_urbsubs_city_m2 <- glmmTMB((rel_growth_rate^(1/3))*100 ~ 
                                 Block +
                                 Year +
                                 (1|Population/Family) +
                                 City_dist + Transect_ID,
...
# test for gxe intxns
# gxe_test_rgr3 <- glmmTMB((rel_growth_rate^(1/3))*100 ~
#                              City_dist*Block +
#                              (1|Population/Family) +
#                              City_dist*Transect_ID,
#                          data = heights %>%
...
#                          REML = F)
# 
# test_gxe(rgr_urbsubs_city_m1, gxe_test_rgr3) # stat equiv
# car::Anova(gxe_test_rgr3, type = "III") # qual identical results
# car::Anova(rgr_urbsubs_city_m1, type = "III")
```

#### Urb_score
```{r}
rgr_urbsubs_usc_m1 <- glmmTMB((rel_growth_rate^(1/3))*100 ~ 
                                 Block +
                                 Year +
                                 (1|Population/Family) +
                                 Urb_score * Transect_ID,
...
# MAIN EFFECTS MODEL
rgr_urbsubs_usc_m2 <- glmmTMB((rel_growth_rate^(1/3))*100 ~ 
                                 Block +
                                 Year +
                                 (1|Population/Family) +
                                 Urb_score + Transect_ID,
...
# # test for gxe intxns
# gxe_test_rgr4 <- glmmTMB((rel_growth_rate^(1/3))*100 ~
#                              Urb_score*Block +
#                              (1|Population/Family) +
#                              Urb_score*Transect_ID,
#                          data = heights %>%
...
# car::Anova(rgr_urbsubs_usc_m1, type = "III") # shows urb_score*block intxn is sig (p = 0.0337)
# car::Anova(gxe_test_rgr4, type = "III")
# 
# # compare ranovas
# # have to use lmer instead of glmmtmb- ranova can't use glmmtmb- otherwise identical models
# # GxE MOD
# ranova(lmer((rel_growth_rate^(1/3))*100 ~
#                              Urb_score*Block +
#                         #     Year +
#                              (1|Population/Family) +
#                              Urb_score*Transect_ID,
