...
# save list of models as herbivore_models
herbivore_models <- all_models
```
...
## Herbivores
```{r}
herbivore_city_gr <- herbivore_models %>%
  purrr::flatten() %>%
  keep(grepl('gr', names(.))) %>%
  keep(grepl('City', names(.)))

names(herbivore_city_gr) <- c(
"monarch_mods"          ,
"asclepiadis_mods"         ,
"clivicollis_mods")
```
...
all_blups %<>%
  ...
  dplyr::relocate('Pollinaria removed',
                  .before = 'Follicles') %>%
...
all_blups %<>%
  # arrange traits by life history category
  dplyr::relocate('Ramets \n before flowering',
                  .after = 'Relative growth rate') %>%
  dplyr::relocate('Ramets \n after flowering',
                  .after = 'Ramets \n before flowering') %>%
  dplyr::relocate('Mortality',
                  .after = 'Ramets \n after flowering')
```
