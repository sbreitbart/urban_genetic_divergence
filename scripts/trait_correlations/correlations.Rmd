...
# back-transform traits transformed in lmers
  dplyr::mutate(mean_poll =          .[[7]]^2, # inverse of sqrt(x)
                ...
                SLA =                exp(.[[12]]), # inverse of log
                Total_height_early = .[[13]]^3, # inverse of x^(1/3)
                Total_height_late =  .[[14]]^3, # "
                Rel_growth_rate =    .[[15]]^3, # "
                Latex =              .[[19]]^3, # "
                Herbivory_mean_early=exp(.[[21]]), # inverse of log
                Herbivory_mean_late =exp(.[[23]]), # "
                Scar_length_cm =     exp(.[[25]])) %>% # "
...
all_blups %<>%
  ...
  dplyr::relocate('Weevil damage (quantitative)',
                  .after = 'Weevil damage (binary)') %>%
  dplyr::relocate('Total cardenolide \nconcentration',
                  .after = 'Weevil damage (quantitative)')
```
...
# Visualize
LEAVING OFF HERE 12/9/2022- This probably won't be in final paper and the correlation matrix won't plot- maybe because some values are NA/Inf? So this isn't finished
## Scatterplot matrix
```{r}

correlations2.r[correlations2.r == 1] <- NA # replace all perfect correlations (1; var A on var A; var B on var B; etc.) with NAs

# remove traits with values that are nearly zero: SLA
correlations2.r %>%
  dplyr::select(-SLA) %>%
  dplyr::filter(-SLA)
...
dev.copy2pdf(file = here::here("./Figures_Tables/correlations/loess_scatterplot_matrix.pdf"),
             width = 20, height = 20)
```
...
dev.copy2pdf(file = here::here("./Figures_Tables/correlations/heatmap_with_p_original_new palette.pdf"),
             width = 20, height = 20)
...
dev.copy2pdf(file = here::here("./Figures_Tables/correlations/heatmap_with_p_asterisks_original.pdf"),
             width = 20, height = 20)
...
dev.copy2pdf(file = here::here("./Figures_Tables/correlations/heatmap_with_r_original.pdf"),
             width = 20, height = 20)
```
...
dev.copy2pdf(file = here::here("./Figures_Tables/correlations/heatmap.pdf"),
             width = 14, height = 14)
```
