...
extract_p_Q2 <- function(model){
 response_var <- tidy_anova(model) %>%
   dplyr::filter(
     Predictor == "Distance to City Center x Subtransect" |
     Predictor == "Urbanization Score x Subtransect"  |
     Predictor == "Subtransect") %>%
   dplyr::slice_min(p) %>% # take smallest p-value
    dplyr::select(Response) %>%
  as.character()
...
# Perform p-value adjustment and add to df-----
  dplyr::mutate(
    ...
    BH = p.adjust(
      p,
      method = "BH"), # Benjamini-Hochberg procedure
    Holm = p.adjust(
      p,
      method = "holm"),
    Hochberg = p.adjust(
    ...
    BY = p.adjust(
      p,
      method = "BY")  # stands for Benjamini, Hochberg, and Yekutieli procedure
    )
...
make_FDR_table <- function(p_val_df){
  ...
  dplyr::rename("Original p" = "p",
    ...
    str_detect(Variable, "cardenolide_Total")~
      "Total cardenolides",
...
#### New order, because now the updated Officer package has a bug and prints these in the reverse order
```{r}
...
t1_results_pop <- binom.test(t1_pop_succ, t1_pop_trials, prob_succ, "greater")
...
t1_results_fam <- binom.test(t1_fam_succ, t1_fam_trials, prob_succ, "greater")
...
t2_results_pop <- binom.test(t2_pop_succ, t2_pop_trials, prob_succ, "greater")
...
t2_results_fam <- binom.test(t2_fam_succ, t2_fam_trials, prob_succ, "greater")

t2_results_fam$p.value # p-value:  4.010664e-06
t2_results_fam$estimate # probability of success: 0.3333333    
```
...
t3_results_pop <- binom.test(t3_pop_succ, t3_pop_trials, prob_succ, "greater")

t3_results_pop$p.value # p-value: 0.1504944 
t3_results_pop$estimate # probability of success: 0.1111111    
```
...
t3_results_fam <- binom.test(t3_fam_succ, t3_fam_trials, prob_succ, "greater")
...
t1_results_pop2 <- binom.test(t1_pop_succ, t1_pop_trials, prob_succ, "greater")

t1_results_pop2$p.value # p-value: 1
...
t1_results_fam2 <- binom.test(t1_fam_succ, t1_fam_trials, prob_succ, "greater")
...
t2_results_pop2 <- binom.test(t2_pop_succ, t2_pop_trials, prob_succ, "greater")

t2_results_pop2$p.value # p-value: 1
...
t2_results_fam2 <- binom.test(t2_fam_succ, t2_fam_trials, prob_succ, "greater")
...
# Q3 results
binomial_results_q3 <- data.frame(
