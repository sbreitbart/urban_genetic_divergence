...
# city_dist
poll_mods_best_c[1]  # sqrt transformed
poll_gradient_01_pred <- ggeffects::ggpredict(poll_mods_best_c[[1]] ,
                                   terms = c("City_dist"),
                                   type = "fe") %>%
  as.data.frame() 

# urb_score
poll_mods_best_u[1]  # sqrt transformed
poll_gradient_02_pred <- ggeffects::ggpredict(poll_mods_best_u[[1]],
                                   terms = c("Urb_score"),
                                   type = "fe") %>%
  as.data.frame()
```
...
# city_dist
poll_mods_best_c[2]  # sqrt transformed
poll_urbsubs_01_pred <- ggeffects::ggpredict(poll_mods_best_c[[2]] ,
                                   terms = c("City_dist", "Transect_ID"),
                                   type = "fe") %>%
  as.data.frame()

# urb_score
poll_mods_best_u[2]  # sqrt transformed
poll_urbsubs_02_pred <- ggeffects::ggpredict(poll_mods_best_u[[2]] ,
                                   terms = c("Urb_score", "Transect_ID"),
                                   type = "fe")
```
...
# adding stats from ANOVA
           ggpp::geom_text_npc(aes(npcx = 0.05, npcy = 0.06,
                    label = paste0(" Distance: p = ", tidy_anova(poll_mods_best_c[[1]]) %>%
dplyr::filter(Predictor == "Distance to City Center") %>%
dplyr::select(p))),
size = 4.5)
...
# adding stats from ANOVA
           ggpp::geom_text_npc(aes(npcx = 0.05, npcy = 1,
                    label = paste0(" Distance: p = ", tidy_anova(first_pods_mods_best_c[[1]]) %>%
dplyr::filter(Predictor == "Distance to City Center") %>%
dplyr::select(p))),
size = 4.5)
...
# adding stats from ANOVA
           ggpp::geom_text_npc(aes(npcx = 0.05, npcy = 0.06,
                    label = paste0(" Urb. Score: p = ", tidy_anova(poll_mods_best_u[[1]]) %>%
dplyr::filter(Predictor == "Urbanization Score") %>%
dplyr::select(p))),
size = 4.5)
...
# adding stats from ANOVA
           ggpp::geom_text_npc(aes(npcx = 1, npcy = 0.96,
                    label = paste0(" Distance: p = ", tidy_anova(poll_mods_best_c[[2]]) %>%
dplyr::filter(Predictor == "Distance to City Center") %>%
dplyr::select(p))),
size = 4.5) +
  ggpp::geom_text_npc(aes(npcx = 1, npcy = 0.88,
                    label = paste0(" Transect: p = ", tidy_anova(poll_mods_best_c[[2]]) %>%
dplyr::filter(Predictor == "Subtransect") %>%
dplyr::select(p))),
size = 4.5)
...
# adding stats from ANOVA
           ggpp::geom_text_npc(aes(npcx = 0.05, npcy = 0.97,
                    label = paste0(" Urb. Score: p = ", tidy_anova(poll_mods_best_u[[2]]) %>%
dplyr::filter(Predictor == "Urbanization Score") %>%
dplyr::select(p))),
size = 4.5) +
  ggpp::geom_text_npc(aes(npcx = 0.05, npcy = 0.88,
                    label = paste0(" Transect: p = ", tidy_anova(poll_mods_best_u[[2]]) %>%
dplyr::filter(Predictor == "Subtransect") %>%
dplyr::select(p))),
size = 4.5)
