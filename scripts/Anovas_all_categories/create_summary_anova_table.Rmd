...
anova_table_city2_simpler %>%
  ...
  theme_box() %>%
  flextable::bold(i = ~ p_dist <= 0.05, j = 3) %>%
  flextable::bold(i = ~ p_sub <= 0.05, j = 5) %>%
  flextable::bold(i = ~ p_intx <= 0.05, j = 7) %>%
  ftExtra::separate_header() %>%
  merge_v(j = ~Variable) %>% 
  align(j = 2:7, align = "center", part = "all") %>%
  hline(border = NULL, part = "body") %>%
  flextable::compose(i = 1, j = c(2,4,6), part = "header",
                     value = as_paragraph("χ", as_sup("2"))) %>%
  merge_h(i = 2, part = "header") %>%
  flextable::compose(i = 2, j = 2, part = "header",
                     value = as_paragraph("Distance to City Center")) %>%
  flextable::compose(i = 2, j = 4, part = "header",
                     value = as_paragraph("Subtransect")) %>%  flextable::compose(i = 2, j = 6, part = "header",
