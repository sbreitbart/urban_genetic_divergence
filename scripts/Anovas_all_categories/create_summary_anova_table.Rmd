...
full_join(
  ...
  dplyr::mutate(ID = row_number()+(row_number()*1)-1),
  
  anova_table_usc %>%
  dplyr::filter(Predictor == "Urbanization Score") %>%
  dplyr::mutate(ID =  row_number()+(row_number()*1))
  ...
  flextable::save_as_docx(.,
                          path = here::here("./Figures_Tables/ANOVA_1yr/ANOVA_1yr_DistAndUrbsc.docx"))
...
### Combine city_dist and Urb_score, remove block
```{r}
# get IDs used to order variables
orders <- anova_table_city2 %>%
  dplyr::filter(Predictor == "Subtransect") %>%
  dplyr::mutate(ID = row_number()) %>%
  dplyr::group_by(Variable) %>%
  dplyr::summarise(Variable = first(Variable),
                   ID = first(ID),
                   ID2 = row_number()) %>%
  dplyr::filter(ID2 == 1) %>%
  dplyr::select(-ID2) %>%
...
full_join(
  ...
  dplyr::filter(Predictor != "Block" &
                Predictor != "(Intercept)" &
                Predictor != "Sample") %>%
  dplyr::mutate(ID2 = case_when(
    Predictor == "Distance to City Center" ~ 1,
    Predictor == "Subtransect" ~ 2,
    Predictor == "Distance to City Center x Subtransect" ~ 3,
    TRUE ~ 10)),
  ...
  dplyr::filter(Predictor != "Block" &
                ...
                Predictor != "Sample") %>%
  dplyr::mutate(ID2 = case_when(
    Predictor == "Urbanization Score" ~ 4,
    Predictor == "Subtransect" ~ 5,
    Predictor == "Urbanization Score x Subtransect" ~ 6,
    TRUE ~ 10))
