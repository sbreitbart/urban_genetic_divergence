...
library(reshape) # not installed on this machine # not installed on this machine
library(reshape2) # not installed on this machine
...
library(redres) # not installed on this machine
...
library(lindia) # not installed on this machine
library(e1071) # not installed on this machine
library(ggpubr) # 'ggplot2' Based Publication Ready Plots
library(forcats) # Tools for Working with Categorical Variables (Factors)
library(car) # Companion to Applied Regression # Companion to Applied Regression
library(multcomp) # Simultaneous Inference in General Parametric Models
library(lsmeans) # Least-Squares Means
library(ggbiplot) # not installed on this machine
library(ggplot2) # Create Elegant Data Visualisations Using the Grammar of Graphics # Create Elegant Data Visualisations Using the Grammar of Graphics # Create Elegant Data Visualisations Using the Grammar of Graphics
...
require(ggiraph) # not installed on this machine
require(ggiraphExtra) # not installed on this machine
...
# recode plants as having experienced  herbivory or not
...
herbivory2020_familymeans <- herbivory_both_2020 %>%
  group_by(Population, Family) %>%
    dplyr::summarise(
      ...
      Herbivory.Sept_mean = mean(Herbivory.Sept_mean, na.rm = T),
      Herbivory.July_mean_binary = mean(Herbivory.July_mean_binary, na.rm = T),
      Herbivory.Sept_mean_binary = mean(Herbivory.Sept_mean_binary, na.rm = T),
      City_dist = first(City_dist),
      Transect_ID = first(Transect_ID),
      Urb_score = first(Urb_score),
      nPlants=n())
...
### Blocks in plot
```{r}
weevil_both$Block <- as.factor(weevil_both$Block 
)
ggplot(data =  weevil_both, aes(x = Column, y = Row)) + 
      geom_tile(aes(fill = Block), colour = "black") +
      scale_y_reverse() +
  theme_minimal()

ggsave(here::here("./Figures_Tables/Spatial_patterns/blocks.png"),
         width = 18, height = 8, units = "cm")
```

### Origin of plants (urban, rural)
```{r}
# ALL PLANTS
ggplot(data =  heights_both_2020, aes(x = Column, y = Row)) + 
      geom_tile(aes(fill = Transect_ID), colour = "black") +
      scale_y_reverse() +
  # theme_minimal()
  theme_pubclean() +
...
ggsave(here::here("./Figures_Tables/Spatial_patterns/all_plants.png"),
         width = 18, height = 8, units = "cm")


# JUST URBAN PLANTS
ggplot(data =  heights_both_2020 %>% filter(., Urb_Rur == "Urban"), aes(x = Column, y = Row)) + 
      geom_tile(aes(fill = Urb_Rur), colour = "black") +
      scale_y_reverse() +
  # theme_minimal()
  theme_pubclean() +
...
ggsave(here::here("./Figures_Tables/Spatial_patterns/urban_plants.png"),
         width = 18, height = 8, units = "cm")
```
 ...
 # scale_shape(aes(x = City_dist, y = mean_height_Sept), solid = F, data = heights_both2020_popmeans)+
  ...
  scale_fill_manual(values = c("blue", "orange"),
                     breaks = c("June", "September"),
                       labels = c("June", "September"),
                       name = "Legend")
ggsave(here::here("./Figures_Tables/Figures/2020/2020_heights_allsites_distance.png"),
 ...
 # scale_shape(aes(x = City_dist, y = mean_height_Sept), solid = F, data = heights_both2020_popmeans)+
  ...
  scale_fill_manual(values = c("blue", "orange"),
                     breaks = c("June", "September"),
                       ...
                       name = "Legend") +
  scale_x_reverse()
ggsave(here::here("./Figures_Tables/Figures/2020/2020_heights_allsites_urbscore.png"),
         width = 14, height = 8, units = "cm")
```
...
# Combine city_dist figures into one
ggarrange(
  (ggplot(heights_both2020_popmeans %>% filter(Transect_ID != "Rural"), aes(x = City_dist, y = mean_height_June)) + 
  labs(x = "Distance to urban center (km)", 
    y = "June: Mean height (cm)") +
...
ggsave(here::here("./Figures_Tables/Figures/2020/2020_heights_urbansites_distance.png"),
         width = 14, height = 8, units = "cm")
```
...
# sjPlot::plot_model(weev_urbsubs_city_m11_ME)
ggplot(weevil_both2020_popmeans %>% filter(Transect_ID != "Rural"), aes(x = Transect_ID, y = mean_scar_div_Juneheight)) + 
  geom_boxplot(aes(color = Transect_ID), outlier.shape = NA) +
  geom_jitter(aes(color = Transect_ID),width = 0.1) +
  theme_pubr()
...
ggplot(weevil_both2020_popmeans %>% filter(Transect_ID != "Rural"), aes(x = City_dist, y = mean_scar)) + 
  labs(x = "Distance to urban center (km)", 
    y = "Total weevil scar length") +
  theme_urbansubs_1
ggsave(here::here("./Figures_Tables/Figures/2020/2020_weevdamage_urbansites_distance.png"),
       width = 18, height = 8, units = "cm")
...
ggsave(here::here("./Figures_Tables/Figures/2020/2020_weevtotal_urbansites_distance.png"),
         width = 14, height = 8, units = "cm")
```
## Herbivory
### Regressions
...
# LOGISTIC REGRESSION: IMPORT PLANT-LEVEL DATA FROM MAIN SCRIPT FIRST (not done in this script yet)
ggplot() + 
  labs(x = "Distance to urban center (km)", 
    y = "Mean Herbivory (Binary)") +
  geom_point(aes(x = City_dist, y = Herbivory.July_mean_binary, fill = 'July', shape = 'July'), size=2, data = herbivory2020_popmeans) +
  geom_point(aes(x = City_dist, y = Herbivory.Sept_mean_binary, fill = 'September', shape = 'September' ), size=2, data = herbivory2020_popmeans) +
  xlim(0, 71) +
  # ylim(0, 120) +
  geom_smooth(aes(x = City_dist, y = Herbivory.July_mean_binary, fill = 'July', color = 'July'), size=0.75, method = lm, alpha=.15, se = T, data = herbivory2020_popmeans) +
  geom_smooth(aes(x = City_dist, y = Herbivory.Sept_mean_binary, fill = 'September', color = 'September'), size=0.75, method = lm, alpha=.15, se = T, data = herbivory2020_popmeans) +
  theme_bw() +
  theme(text = element_text(size=12)) +
  scale_shape_manual(values = c(21, 24),
  ...
  scale_fill_manual(values = c("blue", "orange"),
                     breaks = c("July", "September"),
                       labels = c("July", "September"),
                       name = "Legend") 
ggsave(here::here("./Figures_Tables/Figures/2020/2020_herbiv_bin_allsites_distance.png"),
         width = 14, height = 8, units = "cm")
...
# split by subtransect: city_dist
ggplot(herbivory2020_popmeans %>% filter(Transect_ID != "Rural"),
       aes(x = City_dist, y = Herbivory.July_mean)) + 
  labs(x = "Distance to urban center (km)", 
    y = "Mean Herbivory") +
  theme_urbansubs_1 +
  xlim(0, 35) 
ggsave(here::here("./Figures_Tables/Figures/2020/2020_herbiv_urbansites_distance.png"),
       width = 18, height = 8, units = "cm")
  
# split by subtransect: urb-score
 ...
 # scale_shape(aes(x = City_dist, y = mean_height_Sept), solid = F, data = heights_both2020_popmeans)+
  ...
  scale_fill_manual(values = c("blue", "orange"),
                     breaks = c("June", "September"),
                       labels = c("June", "September"),
                       name = "Legend")
ggsave(here::here("./Figures_Tables/Figures/2020/2020_Ramets_allsites_distance.png"),
         width = 14, height = 8, units = "cm")
```
...
ggplot(heights_both2020_popmeans, aes(x = City_dist, y = mean_rel_growth_rate)) + 
  ...
  xlim(0, 71) +
  ylim(-0.01, 0.015)
ggsave(here::here("./Figures_Tables/Figures/2020/2020_RGR_allsites_distance.png"),
         width = 14, height = 8, units = "cm")
...
ggplot(heights_both2020_popmeans %>% filter(Transect_ID != "Rural"), aes(x = City_dist, y = mean_rel_growth_rate)) + 
  ...
  theme_urbansubs_1 +
  xlim(0, 35)
ggsave(here::here("./Figures_Tables/Figures/2020/2020_RGR_urbansites_distance.png"),
         width = 14, height = 8, units = "cm")
