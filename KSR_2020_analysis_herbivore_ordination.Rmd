...
m1_DP <- glmer(Monarch_Quantity_Observed ~
              ...
              City_dist 
              , data = herbivores , family = poisson) #singular
...
```{r}
# CITY DISTANCE
m1_DP.urb <- glmer(Monarch_Quantity_Observed ~
              (1|Block) + (1|Population/Family) + (1|Sample) +
...
m1_DP.urb_ME <- glmer(Monarch_Quantity_Observed ~
              (1|Block) + (1|Population/Family) + (1|Sample) +
              City_dist + Transect_ID,
              data = herbivores %>% 
                filter(., Transect_ID != "Rural"),
...
# URBANIZATION SCORE
m2_DP.urb <- glmer(Monarch_Quantity_Observed ~
              (1|Block) + (1|Population/Family) + (1|Sample) +
              ...
              control=glmerControl(optimizer="bobyqa",
                                 optCtrl=list(maxfun=2e5))) # singular
res <- simulateResiduals(m2_DP.urb)
plot(res)
# looks good


m2_DP.urb_ME <- glmer(Monarch_Quantity_Observed ~
              (1|Block) + (1|Population/Family) + (1|Sample) +
              Urb_score + Transect_ID,
              data = herbivores %>% 
                filter(., Transect_ID != "Rural"),
...
## Tetraopes tetropthalmus- marg sig along gradient w/urbanization score ONLY
### Diagnostics
#### All sites
```{r}
# DISTANCE FROM CITY CENTER
...
# URBANIZATION SCORE
m2_TT <- glmer(Tetraopes_tetropthalmus ~
              (1|Block) + (1|Population/Family) + (1|Sample) +
              Urb_score 
              , data = herbivores , family = poisson,
              control=glmerControl(optimizer="bobyqa",
...
m1_TT.urb_ME <- glmer(Tetraopes_tetropthalmus ~
              (1|Block) + (1|Population/Family) + (1|Sample) +
              City_dist + Transect_ID,
              data = herbivores %>% 
                filter(., Transect_ID != "Rural"),
...
# URBANIZATION SCORE
m2_TT.urb <- glmer(Tetraopes_tetropthalmus ~
              (1|Block) + (1|Population/Family) + (1|Sample) +
              Urb_score * Transect_ID,
              data = herbivores %>% 
                filter(., Transect_ID != "Rural"),
...
m2_TT.urb_ME <- glmer(Tetraopes_tetropthalmus ~
              (1|Block) + (1|Population/Family) + (1|Sample) +
              Urb_score + Transect_ID,
              data = herbivores %>% 
                filter(., Transect_ID != "Rural"),
...
# very small r-sq.
```
## Euchaetes egle- sig diff along gradient for urban sites w/urbanization score. High r-squared too!
### Diagnostics
#### All sites
```{r}
# DISTANCE FROM CITY CENTER
...
plot(res)
testDispersion(m1_RL.ZI)
# Looks good enough. Using this model

# trying to remove random effects
m1_RL.1 <- glmer(Rhyssomatus_lineaticollis_bin ~
              (1|Population:Family) + 
              City_dist,
              data = herbivores,
              family = binomial,
              control=glmerControl(optimizer="bobyqa",
                                 optCtrl=list(maxfun=2e5)))
res <- simulateResiduals(m1_RL.1)
plot(res)
# not converging

# city_dist never anywhere near sig
car::Anova(m1_RL)
car::Anova(m1_RL.ZI)
car::Anova(m1_RL.1)
...
testDispersion(m2_RL) # better
testDispersion(m2_RL.ZI)

testZeroInflation(m2_RL) # better
testZeroInflation(m2_RL.ZI) 



# urb score never anywhere near sig
car::Anova(m2_RL)
car::Anova(m2_RL.ZI)
...
```{r}
# DISTANCE FROM CITY CENTER
m1_RL.urb <- glmer(Rhyssomatus_lineaticollis_bin ~
              (1|Block) + (1|Population/Family) + (1|Sample) +
              City_dist * Transect_ID,
              data = herbivores %>% 
                filter(., Transect_ID != "Rural"),
...
m1_RL.urb_ME <- glmer(Rhyssomatus_lineaticollis_bin ~
              (1|Block) + (1|Population/Family) + (1|Sample) +
              City_dist + Transect_ID,
              data = herbivores %>% 
                filter(., Transect_ID != "Rural"),
...
car::Anova(m1_RL.urb)
car::Anova(m1_RL.urb_ME) #city_dist sig
...
# URBANIZATION SCORE
m2_RL.urb <- glmer(Rhyssomatus_lineaticollis_bin ~
              (1|Block) + (1|Population/Family) + (1|Sample) +
              Urb_score * Transect_ID,
              data = herbivores %>% 
                filter(., Transect_ID != "Rural"),
...
plot(res)
# Looks good though singular.. trying ZI just in case


m2_RL.urb.ZI <- glmmTMB(Rhyssomatus_lineaticollis_bin ~
              (1|Block) + (1|Population/Family) + (1|Sample) +
              Urb_score * Transect_ID,
              data = herbivores %>% 
                filter(., Transect_ID != "Rural"),
              ...
              control=glmerControl(optimizer="bobyqa",
                                 optCtrl=list(maxfun=2e5)))
              # CAN'T RUN THIS ???
res <- simulateResiduals(m2_RL.urb.ZI)
plot(res)


car::Anova(m2_RL.urb) # urb score not close to sig



# just main effects model:
# m2_RL.urb_ME <- glmer(Rhyssomatus_lineaticollis ~
#               (1|Block) + (1|Population/Family) + (1|Sample) +
#               Urb_score + Transect_ID,
#               data = herbivores %>% 
#                 filter(., Transect_ID != "Rural"),
...
Anova_etc( m1_RL.nb,
           m1_RL.urb,
           m1_RL.urb_ME,
           , # ? don't know which model to make or use
           m2_RL.urb, # try neg binomial too (m2_RL.urb.nb)
           m2_RL.urb_ME) # try neg binomial too
...
```{r}
# ggplot: limit y axis
ggplot(herbivores2020_popmeans %>% filter(Transect_ID != "Rural"), aes(x = Urb_score, y = mean_Liriomyza_asclepiadis)) + 
  labs(x = "Urbanization Score", 
    y = "Mean Liriomyza asclepiadis count") +
  ylim(0,20) +
  theme_urbansubs_1 +
  xlim(-2, 4)
...
# 
# library(stargazer)
# stargazer(car::Anova(m2_LA),
#           car::Anova(m2_LA.urb),
#           type = "latex",
#           out ="iristest.html")

# https://stackoverflow.com/questions/62954069/r-anova-output-of-several-models-side-by-side
```
