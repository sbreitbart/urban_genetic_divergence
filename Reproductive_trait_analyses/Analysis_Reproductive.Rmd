...
# Date of first follicle-----
# 2020-----
firstpod_ranova <- glmer(Julian_first_follicle^3 ~
                               Block +
                               (1|Population) +
                               (1|Population:Fam_uniq),
                             data = flowering %>%
...
# 2021-----
firstpod_ranova2 <- glmer(Julian_first_follicle^3 ~
                               Block +
                               (1|Population) +
                               (1|Population:Fam_uniq),
                             data = flowering %>%
...
# 2022-----
firstpod_ranova3 <- glmer(Julian_first_follicle^2 ~
                               Block +
                               (1|Population) +
                               (1|Population:Fam_uniq),
                             data = flowering %>% 
...
# then rescale variable
# use lmer b/c some values neg now= can't use poisson
dfs <- flowering %>%
                    dplyr::filter(is.na(
                      Julian_first_follicle) == F &
                        Transect_ID != "Rural" &
                        Year == 2022)
dfs[,"Julian_first_follicle"] <- scale(dfs[,'Julian_first_follicle'])

# EVEN THIS IS STILL SINGULAR
m1_sc <- lmer(Julian_first_follicle ~
                           #    Block +
                               (1|Population), #+
                           #    (1|Population:Fam_uniq),
                             data = dfs,
...
# this has LRT of 0, p of 1 for both pop and fam
ranova(m1_sc)




# same output... p = 1, chisq = 0
pb_ranova_transects(m1_sc,
          "Date of first follicle: 2022",
          "./Reproductive_trait_analyses/Tables/Ranova/Date_first_follicle_2022_transects_recentering.docx")
