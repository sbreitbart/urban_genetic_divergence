...
# city_dist
first_pods_mods_best_c[1]  # squared
firstpods_gradient_01_pred <- ggeffects::ggpredict(first_pods_mods_best_c[[1]] ,
                                   terms = c("City_dist"),
                                   type = "fe") %>%
  as.data.frame() %>%
  dplyr::mutate(predicted = sqrt(predicted)) # back-transforming to Julian dates so figure axis makes sense

# urb_score
first_pods_mods_best_u[1] 
...
# city_dist
first_pods_mods_best_c[2] 
firstpods_urbsubs_01_pred <- ggeffects::ggpredict(first_pods_mods_best_c[[2]] ,
                                   terms = c("City_dist", "Transect_ID"),
                                   type = "fe") %>%
  as.data.frame()%>%
  dplyr::mutate(predicted = (predicted)^(1/3)) # back-transforming to Julian dates so figure axis makes sense

# urb_score
first_pods_mods_best_u[2] 
...
ggplot(firstpods_gradient_01_pred) +
  rep_geoms +
  labs(x = "Distance to Urban Center (km)",
       y = "Julian date of first follicle") +
#  ylim(NA, 1) +
  theme_1() +
...
dplyr::select(p))),
size = 4.5) +
  ggpp::geom_text_npc(aes(npcx = 0.05, npcy = 0.78,
                    label = paste0(" U x T: p = ", tidy_anova(flsize_mods_best_u[[2]]) %>%
...
ggpred_Q2.urbsubs_flstart1 <- flowering %>%
  dplyr::filter(Transect_ID != "Rural") %>%
  ggplot(aes(y = Julian_oldest_inflor,
             x = Transect_ID,
             color = Transect_ID,
             fill = Transect_ID)) +
  labs(x = "Transect",
...
size = 4.5) +
  ...
  scale_fill_manual(values = c("#FC8D62",
                               "#66C2A5")) +
  scale_x_discrete(labels=c("North" = "Non-Corridor",
                            "South" = "Corridor"))

ggpred_Q2.urbsubs_flstart1



## URB SCORE ONLY-----
levels(flstart_urbsubs_02_pred_transect$group)

ggpred_Q2.urbsubs_flstart2 <-
ggplot(flstart_urbsubs_02_pred_urbscore) +
  labs(x = "Urbanization Score",
       y = "Julian date of first flower",
       color = "#66C2A5",
       fill = "#66C2A5") +
...
# adding stats from ANOVA
           ggpp::geom_text_npc(aes(npcx = 0.05, npcy = 0.96,
                    label = paste0(" Urb. Score: p = ", tidy_anova(flstart_mods_best_u[[3]]) %>%
dplyr::filter(Predictor == "Urbanization Score") %>%
dplyr::select(p))),
size = 4.5) 
