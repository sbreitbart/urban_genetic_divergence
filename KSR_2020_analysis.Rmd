...
all_models_2020 <- qq(flowering_urbsubs_city_m1, # GET ALL MODELS, ADD HERE
             flowering_urbsubs_city_m1_ME,
             flowering_gr_city_m1,
             Height_Jun20_gr_usc_m2)

# list of all models, for parsing purposes
list1 <- list(flowering_urbsubs_city_m1, # GET ALL MODELS, ADD HERE
             flowering_urbsubs_city_m1_ME,
             flowering_gr_city_m1,
             Height_Jun20_gr_usc_m2)

# get model formulas
all_models_2020_formulas <- sapply(list1, formula) %>%
  sapply(., toString)


test3 <- data.frame(sapply(list1, summary)) %>%
  t() %>%
  as.data.frame() %>%
  dplyr::select(., c(call, family)) 

# Get R-sq values here
library(jtools)
export_summs(list1, digits = 3, to.file = "xlsx", file.name = here::here("./Figures_Tables/ANOVA_tables_images/R_sq_2020.xlsx"))


 # as.data.frame(sapply(list1, jtools::export_summs))


test1 <- data.frame(Model_Name1 = all_models_2020, 
       Formula = all_models_2020_formulas,
       sapply(list1, r.squaredGLMM()[1]))
  # dplyr::rename(., R2m_theor = R2m,
  #               R2c_theor = R2c,
  #               R2c_theor = R2m.1,
  #               R2c_theor = R2c.1,
  #               R2c_theor = R2m.2,
  ...
  #               R2c_theor = R2c.3
  #               ) %>%
  # dplyr::select(-Family.2) %>%
  # filter(!grepl( "function", Family.1)) %>%
  # filter(!grepl( "expression", Family.1))

test1_3_merge <- cbind(test3, test1)

test2 <- data.frame(Model_Name1 = all_models_2020, 
       Formula = all_models_2020_formulas,
       R2 = sapply(list1, r.squaredGLMM),
       Summary = sapply(list1, summary)) %>%
  dplyr::rename(., R2m = R2.1, R2c = R2.2)
